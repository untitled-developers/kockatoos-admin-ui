...
function triggerDanger() {
  confirmDanger({
    ...
    message: '<p>Are you sure you want to delete this record? This cannot be undone.</p>',
    onAccept: () => addLog('Danger → Accepted'),
    onReject: () => addLog('Danger → Rejected'),
  });
}
...
function triggerSuccess() {
  confirmSuccess({
    ...
    message: '<p>Everything looks good. Proceed?</p>',
    onAccept: () => addLog('Success → Accepted'),
    onReject: () => addLog('Success → Rejected'),
  });
}
...
function triggerWarning() {
  confirmWarning({
    header: 'Are you sure?',
    message: '<p>This will affect existing data.</p>',
    onAccept: () => addLog('Warning → Accepted'),
    onReject: () => addLog('Warning → Rejected'),
  });
}
...
function triggerInfo() {
  confirmInfo({
    header: 'Info',
    message: '<p>Please confirm you have read this message.</p>',
    onAccept: () => addLog('Info → Accepted'),
    onReject: () => addLog('Info → Rejected'),
  });
}

const importCode = `import BaseConfirmDialog from "kockatoos-admin-ui/components/BaseConfirmDialog.vue";
// BaseConfirmDialog is already mounted by BaseApp — no need to add it yourself.
// Trigger dialogs via useConfirmDialog:
